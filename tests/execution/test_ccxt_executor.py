...
def test_ccxt_executor_live_gate_and_status_mapping(
...
def test_ccxt_executor_rejects_missing_order_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("LIVE_TRADING_ENABLED", "true")
    monkeypatch.setenv("BROKER_OAUTH_2FA_VERIFIED", "true")
...
def test_ccxt_executor_rejects_without_oauth_2fa(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("LIVE_TRADING_ENABLED", "true")
    monkeypatch.delenv("BROKER_OAUTH_2FA_VERIFIED", raising=False)
    executor = CCXTExecutor(
        create_order=lambda payload: {"id": "CX-1", "status": "open"},
