#!/usr/bin/env python
"""
Comprehensive verification for G7, G8, G9, G10 gates
...
Run from project root: python scripts/verify_g7_g8_g9_g10.py
"""
...
def check_function_size(files: list[Path]) -> list[tuple[str, str, int]]:
    """Check function size and return violations (file, function_name, loc)."""
    ...
    for file_path in files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                # Calculate lines of code (including docstring but excluding blank lines)
                ...
                if loc > 50:
                    relative = file_path.relative_to(ROOT_DIR)
                    violations.append((str(relative), node.name, loc))
    return violations
...
def run_function_size_gate(files: list[Path]) -> tuple[str, bool]:
    """Run function size gate (G10) and return status tuple."""
    gate_name = "G10 - Function Size <= 50 LOC"
    print_section(gate_name)
    violations = check_function_size(files)
    if violations:
        print_error(f"{gate_name}: FAILED")
        print(f"\nFound {len(violations)} function(s) exceeding 50 LOC:\n")
        for file_path, func_name, loc in violations[:20]:
            print(f"  {file_path}:{func_name} = {loc} LOC")
        if len(violations) > 20:
            print(f"\n... and {len(violations) - 20} more violations")
        return gate_name, False

    print_success("All functions <= 50 LOC (PASS)")
    return gate_name, True


def main() -> int:
    """Run G7, G8, G9, G10 gate checks."""
    print("\n" + "=" * 70)
    print("  IATB G7, G8, G9, G10 GATES VERIFICATION")
    print("=" * 70)
