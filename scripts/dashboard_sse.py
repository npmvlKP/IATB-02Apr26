...
_SSE_ENDPOINT = f"{_API_BASE}/events/stream"
_FALLBACK_INTERVAL = 5  # seconds
...
def _check_health() -> str:
    ...
    try:
        with urllib.request.urlopen(f"{_API_BASE}/health", timeout=2, context=None) as r:  # noqa: S310
...
<html><head>
<meta charset="utf-8">
<title>iATB Deployment Dashboard (SSE)</title>
<style>
  body {{
  ...
  .connection-sse {{ background: #238636; color: white; }}
  .connection-polling {{ background: #d29922; color: white; }}
  .connection-offline {{ background: #da3633; color: white; }}
</style>
</head><body>
<h1>iATB Deployment Dashboard</h1>
<p style="color:#8b949e">
  Last update: <span id="timestamp">{timestamp}</span> UTC &nbsp;|&nbsp;
  Connection: <span id="connection-status" class="connection-status connection-offline">
    Connecting...
  </span>
</p>
...
<div class="grid">
  <div class="card">
    ...
    <div class="metric {health_class}" id="health-status">{health_status}</div>
  </div>
  <div class="card">
    <div class="metric-label">Trades Today</div>
    <div class="metric" id="total-trades">{total_trades}</div>
  </div>
  <div class="card">
    <div class="metric-label">Net Notional PnL</div>
    <div class="metric" id="net-pnl">{net_pnl}</div>
  </div>
  <div class="card">
    <div class="metric-label">Buy / Sell</div>
    <div class="metric" id="buy-sell-count">{buy_count} / {sell_count}</div>
  </div>
</div>
  ...
  <div class="card">
    <table>
      ...
      <tr>
        ...
        <td class="{finbert_class}" id="finbert-status">{finbert_status}</td>
      </tr>
      <tr>
        <td>AION-IN-v3</td>
        <td class="{aion_class}" id="aion-status">{aion_status}</td>
      </tr>
      <tr>
        <td>Ensemble Aggregator</td>
        <td class="{aggregator_class}" id="aggregator-status">{aggregator_status}</td>
      </tr>
    </table>
  </div>
  <div class="card">
    <div class="metric-label">Ensemble Status</div>
    <div class="metric {ensemble_class}" id="ensemble-status">{ensemble_status_display}</div>
...
function updateConnectionStatus(status) {{
  ...
  if (status === 'sse') {{
    el.textContent = 'SSE (Real-time)';
  }} else if (status === 'polling') {{
    el.textContent = 'Polling (' + (POLL_INTERVAL / 1000) + 's)';
  }} else {{
...
function handleSSEEvent(event) {{
  ...
  if (data.event_type === 'scan_update') {{
    console.log('Scan update:', data);
    // Update scan-related UI if needed
  }} else if (data.event_type === 'pnl_update') {{
    console.log('PnL update:', data);
    // Update PnL-related UI if needed
  }}
}}
...
function updateUI(data) {{
  updateConnectionStatus(useSSE ? 'sse' : 'polling');
  updateTimestamp();

  // Update metrics
  document.getElementById('total-trades').textContent = data.trades_today;
  document.getElementById('net-pnl').textContent = data.pnl_summary.net_notional_pnl;
  document.getElementById('buy-sell-count').textContent =
    data.pnl_summary.buy_trades + ' / ' + data.pnl_summary.sell_trades;

  // Update health status
  const healthOk = data.engine_health.status === 'ok' || data.engine_health.status === 'degraded';
  const healthEl = document.getElementById('health-status');
  healthEl.textContent = healthOk ? 'ONLINE' : 'OFFLINE';
  healthEl.className = 'metric ' + (healthOk ? 'pass' : 'fail');

  // Update sentiment health
  const sentiment = data.sentiment_health;
  updateStatusClass('finbert-status', sentiment.finbert === 'available');
  updateStatusClass('aion-status', sentiment.aion === 'available');
  updateStatusClass('aggregator-status', sentiment.aggregator === 'available');
  updateStatusClass('ensemble-status', sentiment.ensemble_status === 'operational');
...
function updateStatusClass(elementId, isOk) {{
  const el = document.getElementById(elementId);
  el.className = isOk ? 'status-ok' : 'status-err';
  el.textContent = isOk ? 'Available' : 'Unavailable';
}}
...
function updateTradesTable(trades) {{
  ...
  for (const t of trades) {{
    ...
    const badge = status === 'FILLED' ? 'badge-filled' : 'badge-rejected';
    rows += '<tr>' +
      '<td>' + (t.order_id || '') + '</td>' +
      '<td>' + (t.timestamp_utc || '').substr(0, 19) + '</td>' +
      '<td><b>' + (t.side || '') + '</b></td>' +
      '<td>' + (t.symbol || '') + '</td>' +
      '<td>' + (t.quantity || '') + '</td>' +
      '<td>' + (t.price || '') + '</td>' +
...
def main() -> None:
    print("iATB Deployment Dashboard (SSE-enabled)")  # noqa: T201
    print(f"  URL:  http://localhost:{_PORT}")  # noqa: T201
    print(f"  API:  http://localhost:{_PORT}/api/status")  # noqa: T201
    print(f"  SSE:  {_SSE_ENDPOINT}")  # noqa: T201
    print(f"  Fallback: {_FALLBACK_INTERVAL}s polling")  # noqa: T201
